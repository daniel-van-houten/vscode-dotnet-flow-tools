 ...
 */
export function setupServices(context: vscode.ExtensionContext): ServiceContainer {
  const container = new ServiceContainer();

  // Register core services
  container.register<ILogger>(ServiceKeys.LOGGER, new ConsoleLogger());
  container.register<vscode.ExtensionContext>(ServiceKeys.EXTENSION_CONTEXT, context);
  ...
  container.register(ServiceKeys.PROVIDER_REGISTRY, createProviderRegistry(context));
