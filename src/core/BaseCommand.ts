      ...
      return [
        {
          ...
          run: () =>
            vscode.env.openExternal(
              vscode.Uri.parse(UI.links.copilotMarketplace),
            ),
        },
        {
          label: UI.buttons.selectProvider,
          run: () => vscode.commands.executeCommand(COMMANDS.SELECT_MODEL),
        },
      ];
...
async function handleCopilotModelNotSupported(
  errMsg: string,
): Promise<boolean> {
  if (!errMsg.includes("model_not_supported")) {
    return false;
  }
  await presentError(UI.messages.copilotModelNotSupported, [
    {
      ...
      run: () =>
        vscode.env.openExternal(vscode.Uri.parse(UI.links.copilotFeatures)),
    },
  ]);
  return true;
}

async function handleExtensionConfigError(
...
export abstract class BaseCommand implements ICommand {
  ...
  protected async handleError(error: unknown): Promise<void> {
    ...
    if (await handleCopilotModelNotSupported(message)) {
      return;
    }
    ...
    if (error instanceof ProviderError) {
      if (error.originalError) {
        this.logger.error("Provider original error", error.originalError);
      }
    ...
    if (error instanceof ExtensionError) {
      if (await handleExtensionConfigError(error)) {
        return;
      }
    ...
    await presentError(UI.messages.unexpected, [
      {
        label: UI.buttons.copyDetails,
        run: () => vscode.env.clipboard.writeText(stack ?? message),
      },
    ]);
  }
