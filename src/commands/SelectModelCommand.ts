...
export class SelectModelCommand extends BaseCommand {
  ...
  protected async executeInternal(): Promise<void> {
    ...
    const extensionContext =
      this.serviceContainer.get<vscode.ExtensionContext>("extensionContext");
    ...
    const pickItems = allModels.map((model) => {
      const isCurrentSelection =
        ...
        (model.id === currentModelId || model.name === currentModelId);
    ...
    try {
      ...
      await configService.update(
        "model",
        formatCombinedModel(picked.providerId, picked.id) as any,
      );
    } finally {
      resume();
    }
