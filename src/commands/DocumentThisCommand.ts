...
export class DocumentThisCommand extends BaseCommand {
  ...
  protected async executeInternal(): Promise<void> {
    await this.withProgress("", async (progress, token) => {
      ...
      const shouldContinue = await this.handleProcessingStrategy(
        trace,
        context.provider,
        progress,
      );
      ...
      const result = await this.processDocumentation(
        trace,
        context,
        token,
        progress,
  ...
  private async setupExecutionContext(
    ...
    if (!provider.currentModelId || provider.currentModelId.trim() === "") {
      throw new ValidationError(
        'No model selected. Please select an AI model using the "Select AI Model" command before generating documentation.',
      );
    }

    if (!provider.isInitialized()) {
      throw new ResourceNotFoundError(
        "AI provider is not initialized. Please configure and initialize your AI provider in settings.",
        "ai-provider",
      );
    }
  ...
  private async processDocumentation(
    ...
    return await processDocumentationWithChunking(
      ...
      context.provider,
      {
        className: context.symbolInfo.class.name,
        methodName: context.symbolInfo.method.name,
      },
      businessContext,
      token,
