...
export const chunkingComponents: Partial<ComponentRegistry> = {
  ...
  chunkAnalysisInstructions: {
    content: ({ chunkIndex, totalChunks, className, methodName, chunkPosition }) => {
      const isFirstChunk = chunkPosition === 'first' || chunkIndex === 1;
      const isLastChunk = chunkPosition === 'last' || chunkIndex === totalChunks;
    ...
    content: ({ previousDocument, progressiveContext, chunkPosition }) => {
      ...
      if (progressiveContext?.documentationSummary) {
        ...
        if (summary.keySteps && summary.keySteps.length > 0) {
          content += summary.keySteps.map(step => 
            `- **${step.step}**: ${step.businessLogic}`
          ).join('\n');
        } else {
        ...
        if (progressiveContext.identifiedPatterns && progressiveContext.identifiedPatterns.length > 0) {
          content += stripIndent`
        ...
        if (progressiveContext.pendingQuestions && progressiveContext.pendingQuestions.length > 0) {
          content += stripIndent`
  ...
  /** Progressive context summary for token-efficient state management */
  progressiveDocumentationSummary: {
    content: ({ progressiveContext }) => {
      if (!progressiveContext?.documentationSummary) {
        return '';
      }
      ...
      const summary = progressiveContext.documentationSummary;
      return stripIndent`
        ## Progressive Documentation State

        **Purpose:** ${summary.mainPurpose || 'To be determined'}
        
        **Steps Documented:** ${summary.keySteps?.length || 0}
        **Business Rules:** ${summary.businessRules?.length || 0}
        **Data Transformations:** ${summary.dataTransformations?.length || 0}
        
        **Methods Analyzed:** ${progressiveContext.documentedMethods?.size || 0}
        **Referenced But Not Analyzed:** ${progressiveContext.referencedButNotAnalyzed?.size || 0}
      `;
    }
  },
