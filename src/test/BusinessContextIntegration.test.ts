...
import { IModelProvider, ModelInfo, ModelInvokeParams, ModelResponse } from '../providers/IModelProvider';
...
class MockTokenManager implements ITokenManager {
  ...
  async getTokenUsage(content: string, modelId?: string): Promise<TokenUsage> {
    ...
    let recommendation: 'ok' | 'warning' | 'chunk_required' | 'reduce_content' = 'ok';
...
class MockModelProvider implements IModelProvider {
  ...
  async invoke(
    ...
    if (promptText.includes('## Business Domain Context')) {
      mockResponse = 'Mock documentation response with business context integration';
    }
...
suite('Business Context Integration Tests', () => {
  ...
  suite('End-to-End Business Context Flow', () => {
    test('should pass business context through entire documentation generation pipeline', async () => {
      ...
      const testBusinessContext = 'E-commerce platform specializing in electronics and consumer goods';
  ...
  suite('Business Context in Prompt Templates', () => {
    test('should include business context in single-shot template', () => {
      ...
      assert.ok(prompt.includes('The following context describes the specific business domain'));
      ...
      assert.ok(businessContextIndex < codeTraceIndex, 'Business context should appear before code trace');
    });
    ...
    test('should include business context in consolidation template', () => {
      ...
      const businessContextIndex = prompt.indexOf('## Business Domain Context');
      const consolidationIndex = prompt.indexOf('Final Document Generation');
      assert.ok(businessContextIndex < consolidationIndex, 'Business context should appear before consolidation instructions');
    });
    ...
    test('should omit business context section when empty', () => {
      ...
      assert.ok(prompt.includes('Primary Instruction'));
  ...
  suite('Business Context Positioning in Generated Prompts', () => {
    ...
    test('should maintain proper spacing and formatting around business context', () => {
      ...
      const businessContextSection = prompt.substring(
        ...
        prompt.indexOf('Note: You are receiving')
      );

      // Should have proper newlines and formatting
      assert.ok(businessContextSection.includes('## Business Domain Context\n\n'));
      assert.ok(businessContextSection.includes('Retail chain management system\n\n'));
...
- Premium calculations based on risk assessment`;
      ...
      assert.ok(prompt.includes('- Premium calculations based on risk assessment'));
  ...
  suite('Configuration Integration', () => {
    test('should retrieve business context from configuration service', () => {
      // Arrange
      const testContext = 'Educational platform for online courses and certifications';
    ...
    test('should handle configuration updates for business context', async () => {
      ...
      assert.strictEqual(mockConfiguration._updateCalls[1].value, updatedContext);
    });
  ...
  suite('Error Handling and Edge Cases', () => {
    test('should handle business context with special characters', async () => {
      ...
      const mockTraceContent = 'public void TestMethod() { }';
...
\`\`\``;
      ...
      assert.ok(prompt.includes('> Important: All products require quality certification'));
  ...
  suite('Template Processing Integration', () => {
    test('should process business context component correctly in all templates', () => {
      const testBusinessContext = 'Logistics and supply chain management platform';
      ...
      templates.forEach(templateName => {
        ...
        assert.ok(prompt.includes(testBusinessContext), `Template ${templateName} should include the business context content`);
    ...
    test('should handle template processing when business context is missing from context', () => {
      ...
      assert.ok(prompt.includes('Primary Instruction'), 'Should still include other components');
    });
    ...
    test('should maintain template integrity when business context is empty', () => {
      ...
      assert.ok(!prompt.includes('## Business Domain Context'));
      assert.ok(prompt.includes('## Primary Instruction'));
      assert.ok(prompt.includes('Note: You are receiving the complete code trace'));
      assert.ok(prompt.includes('<code_trace>'));
      
      // Should not have extra blank lines where business context would be
